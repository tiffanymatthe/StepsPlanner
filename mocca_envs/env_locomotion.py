...
class Walker3DStepperEnv(EnvBase):
    ...
    robot_class = Mike
    robot_random_start = True
    robot_init_position = [0, 0.3, 1.32] if robot_class == Walker3D else [0, 0.3, 1.]
    robot_init_velocity = None
    ...
    init_step_separation = 0.70 if robot_class == Walker3D else 0.60
    ...
    def __init__(self, **kwargs):
        ...
        self.behaviors = ["heading_var", "timing_gaits", "to_standstill", "backward", "random_walks_backward", "random_walks", "turn_in_place", "side_step", "one_step_plant", "combine_all", "combine_all_heading", "transition_all", "hopping"]
        self.behavior_timing_thresholds = [1.85, 1.8, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75, 1.75]
        assert len(self.behaviors) == len(self.behavior_timing_thresholds)
        ...
        self.terminal_height_curriculum = np.linspace(0.75, 0.45, N) if self.robot_class == Walker3D else np.linspace(0.65, 0.45, N)
        self.applied_gain_curriculum = np.linspace(1.2, 1.2, N) if self.robot_class == Walker3D else np.linspace(1.0, 1.0, N)
        self.electricity_cost = 4.5 / self.robot.action_space.shape[0]
        self.stall_torque_cost = 0.225 / self.robot.action_space.shape[0]
        self.joints_at_limit_cost = 0.1
        ...
        self.dist_range = {
            "to_standstill": np.array([0.65, 0]) if self.robot_class == Walker3D else np.array([0.55, 0]),
            "random_walks": np.array([0.55, 0.75]) if self.robot_class == Walker3D else np.array([0.55, 0.60]),
            "random_walks_backward": np.array([-0.3, -0.43]),
            "turn_in_place": np.array([0.7, 0.1]),
            "side_step": np.array([0.2, 0.7]) if self.robot_class == Walker3D else np.array([0.2, 0.5]),
            "backward": np.array([0.0, -0.43]),
            "heading_var": np.array([0.65, 0.65]) if self.robot_class == Walker3D else np.array([0.55, 0.55]),
            "timing_gaits": np.array([0.65, 0.65])if self.robot_class == Walker3D else np.array([0.55, 0.55]),
            "one_step_plant": np.array([0.65, 0.65])if self.robot_class == Walker3D else np.array([0.55, 0.55]),
        }
        ...
        } if self.robot_class == Walker3D else {
        ...
        if self.is_rendered and self.robot_class == Mike:
            self.robot.decorate()
    ...
    def generate_step_placements(self):
        ...
        elif self.behaviors[self.behavior_curriculum] == "combine_all_heading":
            self.selected_curriculum = self.np_random.choice(list(range(0,self.max_curriculum+1)))
            # want to exclude "combine_all" for selected behavior_curriculum
    ...
    def set_step_state(self, info_index, step_index):
        ...
        self.rendered_steps[step_index].set_position(pos=pos, left=left)
        new_pos = np.copy(pos)
        new_pos[2] += 0.005
    ...
    def step(self, action):
        ...
        if self.is_rendered or self.use_egl:
            ...
            #     else Colors["lightgrey"]
            # )
            self.rendered_steps[(self.next_step_index-1) % self.rendered_step_count].set_color(full=False) # Colors["lightgrey"])
            self.rendered_steps[self.next_step_index % self.rendered_step_count].set_color(full=True) #Colors["dodgerblue"])
