...
class Walker3DStepperEnv(EnvBase):
    ...
    def reset(self):
        ...
        # else:
        self.train_old = False
        if self.np_random.rand() < min(self.curriculum/self.max_curriculum/2, 0.5):
    ...
    def delta_to_k_targets(self):
        ...
        heading_mask = np.ones(k + j) if self.mask_info["heading"][2] else np.zeros(k + j) # * self.timing_mask_value
