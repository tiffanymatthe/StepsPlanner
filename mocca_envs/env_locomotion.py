...
class Walker3DStepperEnv(EnvBase):
    ...
    def calc_base_reward(self, action):
        ...
        if self.target_reached and self.next_step_index > 2:
            if self.prev_pitch is not None:
                self.contact_bonus += self.prev_pitch - np.abs(self.robot.feet_rpy[self.swing_leg, 1])
            self.prev_pitch = np.abs(self.robot.feet_rpy[self.swing_leg, 1])
    ...
    def calc_feet_state(self):
        ...
        self.is_pitch_ok = np.abs(self.robot.feet_rpy[self.swing_leg, 1]) < 0.2 or self.next_step_index <= 2
        ...
        if self.target_reached:
            ...
            if self.target_reached_count >= 2 and self.is_pitch_ok:
