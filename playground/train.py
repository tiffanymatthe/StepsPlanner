...
def main(_seed, _config, _run):
    ...
    if args.use_wandb:
        run = wandb.init(
            project="WalkerStepperEnv-v0 - Plasticity",
            config=args
        )
    ...
    distiller = Distiller(env_name, env_kwargs, args.seed, args.device, args.num_processes, 250, envs, dummy_env, args.experiment_dir)
    ...
    for iteration in range(num_updates):
        ...
        with torch.no_grad():
            ...
            for i in range(4):
                ...
                if (
                    ...
                    and (
                        np.isnan(avg_heading_err_nanmean)
                        or avg_heading_err_nanmean < (10 * DEG2RAD if (current_curriculum > 0 or args.net is not None) else 25 * DEG2RAD)
                        or (avg_heading_err_nanmean < 11 * DEG2RAD and current_behavior_curriculum == 5 and current_curriculum == 9)
                    )
                    and (
        ...
        if prev_behavior_actor_critic is not None and current_behavior_curriculum > 1 and current_curriculum == 0 and current_iteration < 0: # 500:
            freeze_actor = True
        else:
            freeze_actor = False
            ...
            logger.log_epoch(
                {
                    ...
                    "avg_dist_err": nanmean(dist_metric),
                    "curriculum_metric_0": mean_metric[0],
                    "curriculum_metric_1": mean_metric[1],
                    "curriculum_metric_2": mean_metric[2],
                    "curriculum_metric_3": mean_metric[3],
                    "avg_heading_err_0": heading_metric[0],
                    "avg_heading_err_1": heading_metric[1],
                    "avg_heading_err_2": heading_metric[2],
                    "avg_heading_err_3": heading_metric[3],
                    "avg_timing_met_0": timing_metric[0],
                    "avg_timing_met_1": timing_metric[1],
                    "avg_timing_met_2": timing_metric[2],
                    "avg_timing_met_3": timing_metric[3],
